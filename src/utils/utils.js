...
import { DEFAULT_STOP_SEQUENCES, REASONING_EFFORT_MAP } from '../constants/index.js';
...
export function prepareImageRequest(requestBody) {
  ...
  const originalModel = String(requestBody.model || '');
  const modelLower = originalModel.toLowerCase();

  // 解析分辨率（兼容 -2k/-2K/-4k/-4K）
  let imageSize = '1K';
  if (modelLower.includes('-4k')) {
    imageSize = '4K';
  } else if (modelLower.includes('-2k')) {
    imageSize = '2K';
  } else if (modelLower.includes('-1k')) {
    imageSize = '1K';
  }

  // 解析比例（对齐 gcli2api 的后缀约定）
  ...
  // 去掉模型名里的分辨率/比例后缀，避免上游把它当成“未知模型”
  // 支持形如：gemini-3-pro-image-4k-16x9 / gemini-3-pro-image-16x9-4K
  const removableSuffixes = new Set(['4k', '2k', '1k', '21x9', '16x9', '9x16', '4x3', '3x4', '1x1']);
  const parts = originalModel.split('-');
  while (parts.length > 0 && removableSuffixes.has(parts[parts.length - 1].toLowerCase())) {
    parts.pop();
  }
  requestBody.model = parts.join('-') || originalModel;
